 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-kayn/index
 * @returns eslint-config-kayn/react
 * @returns eslint-config-kayn/typescript/index
 * @returns eslint-config-kayn/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-kayn/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
